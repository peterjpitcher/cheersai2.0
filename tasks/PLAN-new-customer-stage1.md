...
- **PR2** `feat/brand-feature-gates` (2026-09-24): built. Migration `20260924090000_accounts_feature_flags.sql` (not yet applied to production; must be applied before merge because brand loading reads the new columns). `npm run ci:verify` green (2,497 tests). `pauseCampaign` deliberately left ungated so live spend can always be stopped. Event ad campaigns need both paid ads and management import; with only paid ads on, the event import fails closed with a message (acceptable while only The Anchor has paid ads).
