# Review Pack: banner-overlay-impl
...
**Base ref:** `01cb931`
**HEAD:** `9c0588c`
**Diff range:** `01cb931...HEAD`
**Stats:**  49 files changed, 2523 insertions(+), 2495 deletions(-)
...
Implementation of the banner overlay consistency design. Banner is now derived data: labels computed live in the UI via useNowMinute(); JPEG rendered server-side at publish time via a Next.js POST endpoint that the Deno publish-queue Edge Function calls over HTTP (Deno cannot run Sharp). Schema rolled out as Migration 1 (additive + CHECK constraints + validated data copy from prompt_context.bannerConfig) and Migration 2 (drop legacy banner_state machinery). Migrations are committed but NOT YET APPLIED to the live database. Concerns: (a) does the Deno worker correctly fail-loud on render endpoint failure without leaking platform calls? (b) are the duplicated helpers in supabase/functions/publish-queue/{banner-config,banner-label}.ts truly equivalent to src/lib/banner/config.ts + src/lib/scheduling/proximity-label.ts + src/lib/scheduling/campaign-timing.ts? (c) is the CRON_SECRET auth on the new render-banner route timing-safe and correctly validated? (d) does Migration 1's data copy from prompt_context.bannerConfig handle malformed legacy values safely? (e) any banner_state machinery still referenced anywhere?

## Diff (`01cb931...HEAD`)

```diff
diff --git a/package.json b/package.json
index 184acee..0190988 100644
--- a/package.json
+++ b/package.json
@@ -18,7 +18,6 @@
...
+    "jsdom": "^29.1.1",
     ...
     "tsx": "^4.21.0",
     "typescript": "^5",
diff --git a/scripts/ops/cleanup-banner-storage.ts b/scripts/ops/cleanup-banner-storage.ts
new file mode 100644
index 0000000..7121f30
--- /dev/null
+++ b/scripts/ops/cleanup-banner-storage.ts
@@ -0,0 +1,136 @@
+/**
...
[diff truncated at line 1500 — total was 6264 lines. Consider scoping the review to fewer files.]
```
...
_(binary or >200KB —     8939 bytes — not embedded)_

### `tests/fixtures/banner/square-1080.jpg`

_(binary or >200KB —     7205 bytes — not embedded)_

### `tests/fixtures/banner/story-1080-1920.jpg`

_(binary or >200KB —    12509 bytes — not embedded)_
...
[truncated at line 200 — original has 333 lines]
```
