...
export function EventCampaignForm({ mediaLibrary, plannerItems, ownerTimezone, onLibraryUpdate, initialDate, onSuccess }: EventCampaignFormProps) {
  ...
  const suggestions: SuggestedSlotDisplay[] = useMemo(
    ...
    [rawSuggestions, plannerItems, timezoneValue],
  );
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    startTransition(async () => {
      try {
        const response = await handleEventCampaignSubmission({ ...values, bannerDefaults });
                ...
                {managementOptionsLoaded && managementOptions.length ? (
                  <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-[var(--c-ink-3)]">{managementOptions.length} matching events loaded.</p>
                    </div>
                    <Button
                      type="button"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                {form.formState.errors.startTime ? (
                  <p className="text-xs text-[var(--c-claret)]">{form.formState.errors.startTime.message}</p>
                ) : null}
              </div>
            </div>
      ...
      content: (controls: StageAccordionControls) => {
        ...
        return (
          <>
            ...
            <div className="space-y-2">
              <Label htmlFor="event-cta-label">Link goal</Label>
              <select
                id="event-cta-label"
                className="w-full rounded-[var(--r-lg)] border border-[var(--c-line)] bg-[var(--c-card)] p-3 text-sm text-[var(--c-ink-2)] focus:border-[var(--c-line-2)] focus:outline-none"
            ...
            <div className="space-y-2">
              <Label htmlFor="event-link-in-bio-url">Link in bio destination</Label>
              <p className="text-xs text-[var(--c-ink-3)]">
                Used on your CheersAI link-in-bio page (keep UTMs separate from the Facebook link above).
              </p>
              <Input
                id="event-link-in-bio-url"
                type="url"
        ...
        return (
          <div className="space-y-3 rounded-[var(--r-xl)] border border-[var(--c-line)] bg-[var(--c-paper-2)] p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-[var(--c-ink)]">Pick the hype moments</p>
                <p className="text-xs text-[var(--c-ink-3)]">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                ...
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold text-white transition ${useManualScheduleValue
                    ? "border-[var(--c-orange)] bg-[var(--c-orange)] hover:bg-[var(--c-orange-hi)]"
                    : "border-[var(--c-orange-soft)] bg-[var(--c-orange-soft)] opacity-60"
                    }`}
                >
                  Apply suggestions
                </button>
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition ${useManualScheduleValue && manualSlotsPresent
                    ? "border-[var(--c-line-2)] text-[var(--c-ink-2)] hover:border-[var(--c-line-2)] hover:text-[var(--c-ink)]"
                    : "border-[var(--c-line)] text-[var(--c-ink-4)]"
                    }`}
                >
                  Clear all
                </button>
        ...
        <>
          ...
          {generatedItems.length ? (
            <section className="space-y-4">
              ...
              <p className="text-sm text-[var(--c-ink-3)]">Swap media, then approve each draft to place it on the schedule.</p>
