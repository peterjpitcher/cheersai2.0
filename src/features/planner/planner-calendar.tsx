    ...
    <section className="space-y-6">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[960px] space-y-3">
          <div className="hidden grid-cols-7 gap-3 text-xs font-semibold uppercase text-slate-500 sm:grid">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-7">
            {weeks.flat().map(({ date, isCurrentMonth, isToday, items }) => {
              const classes = [
                "flex h-[260px] flex-col gap-3 rounded-2xl border p-3 shadow-sm transition",
                isCurrentMonth ? "bg-white border-slate-200" : "bg-slate-50 border-slate-100 opacity-60",
                isToday ? "border-slate-900 ring-2 ring-slate-900/10" : "",
              ]
                .filter(Boolean)
                .join(" ");
                    ...
                    {items.length ? (
                      items.map((item) => (
                        <Link
                          ...
                          className="group flex flex-col gap-1 rounded-lg border border-slate-200 bg-white/80 px-2 py-2 text-left text-xs shadow-sm transition hover:border-slate-400"
                        >
