...
import {
  Calendar, Clock, Copy, Download, ChevronLeft,
  PartyPopper, Sparkles, Sun, Megaphone, Image
} from "lucide-react";
...
export default async function CampaignPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sortedPosts = campaign.campaign_posts?.sort((a, b) => 
                ...
                {sortedPosts.map((post, index) => {
                  ...
                  return (
                    <div key={post.id} className="card">
                      ...
                      <p className="whitespace-pre-wrap text-text-primary bg-background rounded-soft p-3">
                        {post.content}
                      </p>
                    </div>
                  );
