...
import {
  Calendar, Clock, ChevronLeft, Grid3X3, List,
  PartyPopper, Sparkles, Sun, Megaphone, Image as ImageIcon,
  Check, X, RefreshCw, Copy, Loader2, Shield, Edit2
} from "lucide-react";
...
import GuardrailsModal from "@/components/campaign/guardrails-modal";
...
export default function CampaignClientPage({ campaign }: CampaignClientPageProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          {sortedPosts.length === 0 ? (
            ...
            <div className="space-y-4">
              {sortedPosts.map((post: any, index: number) => {
                ...
                const timingLabel = getTimingLabel(post.post_timing, scheduledDate);
      ...
      {/* Guardrails Modal */}
      {selectedPostForGuardrails && (
        <GuardrailsModal
          isOpen={guardrailsModalOpen}
          onClose={() => {
            setGuardrailsModalOpen(false);
            setSelectedPostForGuardrails(null);
          }}
          postId={selectedPostForGuardrails.id}
          platform={selectedPostForGuardrails.platform}
          content={selectedPostForGuardrails.content}
          existingGuardrails={selectedPostForGuardrails.metadata?.guardrails}
        />
      )}
    </div>
  );
