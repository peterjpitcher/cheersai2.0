...
1. **Reference** — Always-current documentation of architecture, features, schema, and APIs that Codex and humans can consult
2. **Communication** — A two-way channel where the user edits docs to express intent (via `#change-request` tags) and Codex reads them as work instructions
3. **Validation** — Drift detection that warns when code and documentation have diverged
4. **Optimization** — Active code quality analysis that flags performance issues, architectural smells, and tech debt as Codex discovers them during documentation
...
1. Read `AGENTS.md`, `package.json`, `.env.example` — get the project overview
...
Read: AGENTS.md, package.json, .env.example, middleware.ts, src/app/ directory structure, src/lib/supabase/, src/lib/auth/
```
...
This is what turns the vault from passive documentation into an active improvement tool. During every documentation pass (init, sync, or drift detection), Codex should actively evaluate the code it reads and record findings in the `Health/` section.
...
5. **Implement** the changes (following the project's AGENTS.md and workspace standards)
...
Business rules documentation helps Codex understand the "why" behind code decisions and catch violations when code doesn't match stated policy. Each domain area gets its own file.
...
The Obsidian vault complements — never replaces — the project's `AGENTS.md`:

| Content | Lives in | Reason |
|---------|---------|--------|
| Quick project profile, commands, env vars | `AGENTS.md` | Fast reference for Codex at session start |
| Architecture deep-dive, visual diagrams | `Obsidian/Architecture/` | Too detailed for AGENTS.md |
...
`AGENTS.md` is the authority for project configuration and commands. The Obsidian vault is the authority for detailed architecture, feature behavior, schema documentation, and project health.
